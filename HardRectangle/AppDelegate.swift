...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static let launcherAppId = "com.teamtumnus.HardRectangleLauncher"

    private let accessibilityAuthorization = AccessibilityAuthorization()
    private let statusItem = HardRectangleStatusItem.instance
    ...
    func checkForConflictingApps() {
        ...
        for app in runningApps {
            ...
            if let conflictingAppName = conflictingAppsIds[bundleId] {
                AlertUtil.oneButtonAlert(question: "Potential window manager conflict: \(conflictingAppName)", text: "Since \(conflictingAppName) might have some overlapping behavior with HardRectangle, it's recommended that you either disable or quit \(conflictingAppName).")
                break
            }
        }
    ...
    func checkForProblematicApps() {
        ...
        if !problemBundles.isEmpty {
            AlertUtil.oneButtonAlert(question: "Known issues with installed applications", text: "\(displayNameString)\n\nThese applications have issues with the drag to screen edge to snap functionality in HardRectangle.\n\nYou can either ignore the applications using the menu item in HardRectangle, or disable drag to screen edge snapping in HardRectangle preferences.")
    ...
    private func checkLaunchOnLogin() {
        let enabled = LaunchOnLoginPreferenceResolver().resolve {
            NSApp.activate(ignoringOtherApps: true)
            let alert = NSAlert()
            alert.alertStyle = .informational
            alert.messageText = "Launch HardRectangle when you log in?".localized
            alert.informativeText = "HardRectangle can start automatically after you sign in. You can change this at any time in Settings.".localized
            alert.addButton(withTitle: "Enable Launch on Login".localized)
        ...
        if #available(macOS 13.0, *) {
            if LaunchOnLogin.isEnabled != enabled {
            ...
            if !smLoginSuccess {
                if #available(OSX 10.12, *) {
                    os_log("Unable to update launch at login. Attempting one more time.", type: .info)
                }
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        ...
        DispatchQueue.main.async {
            ...
            func confirmExecuteTask(action: String, bundleId: String) -> Bool {
                ...
                // `hardrectangle://execute-task=ignore-app` URL with an arbitrary
                // bundle-id. Without confirmation this silently mutates
                // HardRectangle's `disabledApps` defaults. Skip the prompt only
                // when HardRectangle itself is frontmost (i.e. the user almost
                // certainly clicked this from inside HardRectangle's own UI).
                if NSWorkspace.shared.frontmostApplication == NSRunningApplication.current {
                    return true
                }
                let alert = NSAlert()
                alert.alertStyle = .warning
                alert.messageText = "Allow HardRectangle URL action?".localized
                alert.informativeText = String(format: "An external source asked HardRectangle to perform \"%@\" on app bundle id \"%@\". Allow?".localized, action, bundleId)
