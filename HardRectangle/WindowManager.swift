...
class WindowManager {
    ...
    func recordAction(windowId: CGWindowID?,
        ...
        let newCount: Int
        if let lastHardRectangleAction = AppDelegate.windowHistory.lastHardRectangleActions[windowId],
           lastHardRectangleAction.action == action {
            newCount = incrementCount ? lastHardRectangleAction.count + 1 : lastHardRectangleAction.count
        } else {
            newCount = 1
        }
        
        AppDelegate.windowHistory.lastHardRectangleActions[windowId] = HardRectangleAction(
            action: action,
            subAction: subAction,
            rect: resultingRect,
            count: newCount
    ...
    func execute(_ parameters: ExecutionParameters) {
        ...
        if action == .restore {
            ...
            AppDelegate.windowHistory.lastHardRectangleActions.removeValue(forKey: windowId)
            return
        }
        ...
        var lastHardRectangleAction = windowId.flatMap { AppDelegate.windowHistory.lastHardRectangleActions[$0] }
        
        let windowMovedExternally = currentWindowRect != lastHardRectangleAction?.rect
        
        if windowMovedExternally {
            lastHardRectangleAction = nil
            if let windowId {
                AppDelegate.windowHistory.lastHardRectangleActions.removeValue(forKey: windowId)
            }
        }
        ...
        let calculationParams = WindowCalculationParameters(window: currentWindow, usableScreens: usableScreens, action: action, lastAction: lastHardRectangleAction, ignoreTodo: ignoreTodo)
        ...
        let cooperativeCornerPlan = cooperativeCornerResizePlan(focusedWindowId: windowId,
                                                                ...
                                                                lastHardRectangleAction: lastHardRectangleAction)
