...
class SettingsViewController: NSViewController {
    ...
    @IBAction func toggleHideMenuBarIcon(_ sender: NSButton) {
        ...
        HardRectangleStatusItem.instance.refreshVisibility()
    }
    ...
    @IBAction func toggleDoubleClickTitleBar(_ sender: NSButton) {
        ...
        if newSetting && !TitleBarManager.systemSettingDisabled {
            ...
            let conflictDescriptionText = NSLocalizedString(
                "To let HardRectangle manage the title bar double click functionality, you need to disable the corresponding macOS setting.", tableName: "Main", value: "", comment: "")
    ...
    @IBAction func restoreDefaults(_ sender: Any) {
        // Ask user if they want to restore to HardRectangle or Spectacle defaults
        let currentDefaults = Defaults.alternateDefaultShortcuts.enabled ? "HardRectangle" : "Spectacle"
        ...
        let response = AlertUtil.threeButtonAlert(question: defaultShortcutsTitle, text: currentlyUsingText + currentDefaults, buttonOneText: "HardRectangle", buttonTwoText: "Spectacle", buttonThreeText: cancelText)
        if response == .alertThirdButtonReturn { return }

        //  Restore default shortcuts
        let hardRectangleDefaults = response == .alertFirstButtonReturn
        WindowAction.active.forEach { UserDefaults.standard.removeObject(forKey: $0.name) }
        Defaults.alternateDefaultShortcuts.enabled = hardRectangleDefaults
    ...
    @IBAction func exportConfig(_ sender: NSButton) {
        ...
        savePanel.nameFieldStringValue = "HardRectangleConfig"
