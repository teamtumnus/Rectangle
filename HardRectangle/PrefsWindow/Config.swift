...
extension Defaults {
    static func encoded() -> String? {
        ...
        let config = Config(bundleId: "com.teamtumnus.HardRectangle",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let hardRectangleSupportURL = getSupportDir()?
            .appendingPathComponent("HardRectangle", isDirectory: true) {
            
            let configURL = hardRectangleSupportURL.appendingPathComponent("HardRectangleConfig.json")
                        
            let exists = try? configURL.checkResourceIsReachable()
            if exists == true {
                // Defense-in-depth: any process running as this user can drop
                // a HardRectangleConfig.json in Application Support and have it
                ...
                guard isSafe else {
                    AlertUtil.oneButtonAlert(
                        question: "Refused to load HardRectangleConfig.json",
                        text: "The configuration file at \(path) is a symlink or world-writable. HardRectangle has refused to load it. Remove the file or fix its permissions and try again."
                    )
                ...
                let response = AlertUtil.twoButtonAlert(
                    question: "Apply HardRectangle configuration?",
                    text: "A configuration file was found at \(path). Applying it will overwrite your current HardRectangle shortcuts and preferences. Apply now?",
                    confirmText: "Apply",
                    cancelText: "Discard"
                )
                ...
                do {
                    let newFilename = "HardRectangleConfig\(timestamp()).json"
                    
                    try fm.moveItem(atPath: configURL.path, toPath: hardRectangleSupportURL.appendingPathComponent(newFilename).path)
                } catch {
                    do {
                        try fm.removeItem(at: configURL)
                    } catch {
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove HardRectangleConfig.json from \(hardRectangleSupportURL) after loading.")
                    }
                }
            }
        }
