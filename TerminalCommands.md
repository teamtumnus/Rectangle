...
```bash
defaults write com.teamtumnus.HardRectangle alternateDefaultShortcuts -bool true
```
...
```bash
defaults write com.teamtumnus.HardRectangle subsequentExecutionMode -int 2
```
...
```bash
defaults write com.teamtumnus.HardRectangle centerHalfCycles -int 1
```
...
```bash
defaults write com.teamtumnus.HardRectangle resizeOnDirectionalMove -bool true
```
...
```bash
defaults write com.teamtumnus.HardRectangle stageSize -float <VALUE>
```

To set it to a proportion of your screen's width, set it to a value between 0 and 1.

```bash
defaults write com.teamtumnus.HardRectangle stageSize -float <VALUE_BETWEEN_0_AND_1>
```
...
```bash
defaults write com.teamtumnus.HardRectangle todo -int 1
```
...
```bash
defaults write com.teamtumnus.HardRectangle snapModifiers -int 1048576
```
...
```bash
defaults write com.teamtumnus.HardRectangle almostMaximizeHeight -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.teamtumnus.HardRectangle almostMaximizeWidth -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.teamtumnus.HardRectangle specified -dict-add keyCode -float 8 modifierFlags -float 1966080
```

```bash
defaults write com.teamtumnus.HardRectangle specifiedHeight -float 1050
defaults write com.teamtumnus.HardRectangle specifiedWidth -float 1680
```
...
```bash
defaults write com.teamtumnus.HardRectangle centerProminently -dict-add keyCode -float 8 modifierFlags -float 1835305
```
...
```bash
defaults write com.teamtumnus.HardRectangle topLeftNinth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.teamtumnus.HardRectangle topLeftEighth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.teamtumnus.HardRectangle topLeftThird -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.teamtumnus.HardRectangle doubleWidthRight -dict-add keyCode -float 124 modifierFlags -float 11403555
```
...
```bash
defaults write com.teamtumnus.HardRectangle cascadeActiveApp -dict-add keyCode -float 2 modifierFlags -float 393475
```
...
```bash
defaults write com.teamtumnus.HardRectangle footprintAlpha -float <VALUE_BETWEEN_0_&_1>
```

Change the border width. Default is 2 (used to be 1).

```bash
defaults write com.teamtumnus.HardRectangle footprintBorderWidth -float <NUM_PIXELS>
```

Disable the fade.

```bash
defaults write com.teamtumnus.HardRectangle footprintFade -int 2
```

Change the color.

```bash
defaults write com.teamtumnus.HardRectangle footprintColor -string "{\"red\":0,\"blue\":0.5,\"green\":0.5}"
```

Change the animation duration. The value is a multiplier. Default is 0 (no animation).

```bash
defaults write com.teamtumnus.HardRectangle footprintAnimationDurationMultiplier -float <MULTIPLIER>
```
...
```bash
defaults write com.teamtumnus.HardRectangle centeredDirectionalMove -int 2
```
...
Set either value to `0` to disable HardRectangle's screen-relative limit for that dimension and rely on the application's native minimum size.

```bash
defaults write com.teamtumnus.HardRectangle minimumWindowWidth -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.teamtumnus.HardRectangle minimumWindowHeight -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.teamtumnus.HardRectangle sizeOffset -float <NUM_PIXELS>
```
...
```bash
defaults write com.teamtumnus.HardRectangle curtainChangeSize -int 2
```
...
```bash
defaults write com.teamtumnus.HardRectangle largerWidth -dict-add keyCode -float 30 modifierFlags -float 786432
defaults write com.teamtumnus.HardRectangle smallerWidth -dict-add keyCode -float 33 modifierFlags -float 786432
```
...
```bash
defaults write com.teamtumnus.HardRectangle largerHeight -dict-add keyCode -float 30 modifierFlags -float 917504
defaults write com.teamtumnus.HardRectangle smallerHeight -dict-add keyCode -float 33 modifierFlags -float 917504
```
...
```bash
defaults write com.teamtumnus.HardRectangle smallerShrinksMaximizedHeight -bool true
```

## Disabling window restore when moving windows

```bash
defaults write com.teamtumnus.HardRectangle unsnapRestore -int 2
```
...
```bash
defaults write com.teamtumnus.HardRectangle snapEdgeMarginTop -int 10
defaults write com.teamtumnus.HardRectangle snapEdgeMarginBottom -int 10
defaults write com.teamtumnus.HardRectangle snapEdgeMarginLeft -int 10
defaults write com.teamtumnus.HardRectangle snapEdgeMarginRight -int 10
```
...
```bash
defaults write com.teamtumnus.HardRectangle screenEdgeGapTop -int 10
defaults write com.teamtumnus.HardRectangle screenEdgeGapBottom -int 10
defaults write com.teamtumnus.HardRectangle screenEdgeGapLeft -int 10
defaults write com.teamtumnus.HardRectangle screenEdgeGapRight -int 10
```
...
```bash
defaults write com.teamtumnus.HardRectangle screenEdgeGapTopNotch -int 5
```
...
```bash
defaults write com.teamtumnus.HardRectangle screenEdgeGapsOnMainScreenOnly -bool true
```
...
```bash
defaults write com.teamtumnus.HardRectangle ignoredSnapAreas -int 1
```

To disable the Top Half and Bottom Half snap areas, the bit field would be 1111 0000 0000, or 3840

```bash
defaults write com.teamtumnus.HardRectangle ignoredSnapAreas -int 3840
```
...
```bash
defaults write com.teamtumnus.HardRectangle applyGapsToMaximize -int 2
```

To disable the gaps for "Maximize Height", execute:

```bash
defaults write com.teamtumnus.HardRectangle applyGapsToMaximizeHeight -int 2
```
...
```bash
defaults write com.teamtumnus.HardRectangle sixthsSnapArea -bool true
```
...
```bash
defaults write com.teamtumnus.HardRectangle moveCursor -int 1
```
...
```bash
defaults write com.teamtumnus.HardRectangle missionControlDragging -int 2
```

Change the allowed off-screen distance. The value is in pixels and is tied to the speed. Default is 25.

```bash
defaults write com.teamtumnus.HardRectangle missionControlDraggingAllowedOffscreenDistance -float <DISTANCE>
```

Change the disallowed duration. The value is in milliseconds. Default is 250.

```bash
defaults write com.teamtumnus.HardRectangle missionControlDraggingDisallowedDuration -int <DURATION>
```

## Change the behavior of double-click window title bar

To change the action ([upstream list](https://github.com/rxhanson/Rectangle/blob/master/Rectangle/WindowAction.swift)):

```bash
defaults write com.teamtumnus.HardRectangle doubleClickTitleBar -int <ACTION_ID + 1>
```

To disable restore when double-clicked again:

```bash
defaults write com.teamtumnus.HardRectangle doubleClickTitleBarRestore -int 2
```

To disable double-click window title bar only for specific bundle ids (in example, Outlook):

```bash
defaults write com.teamtumnus.HardRectangle doubleClickTitleBarIgnoredApps -string "[\"com.microsoft.Outlook\"]"
```
...
```bash
defaults write com.teamtumnus.HardRectangle screensOrderedByX -int 1
```
...
```bash
defaults write com.teamtumnus.HardRectangle autoMaximize -int 2
```

To restore the default behavior:

```bash
defaults write com.teamtumnus.HardRectangle autoMaximize -int 0
```

## Attempt to preserve window position when moving to another display

By default, moving a window to the next, previous, or a specific display centers it on the destination display. Enable this to instead try preserving the window's position on the destination. If the previous action was a HardRectangle snap (half, third, maximize, etc.), that snap is replayed on the destination display. If the window was positioned manually, its rect is mapped proportionally from the source display to the destination display (a window at the right third stays at the right third) and clamped so it never overflows. This is off by default.

```bash
defaults write com.teamtumnus.HardRectangle attemptMatchOnNextPrevDisplay -int 1
```

To disable it again:

```bash
defaults write com.teamtumnus.HardRectangle attemptMatchOnNextPrevDisplay -int 2
```
...
```bash
defaults write com.teamtumnus.HardRectangle cyclingOverlapOffset -bool true
```

The default offset is 11pt. To customize the offset size:

```bash
defaults write com.teamtumnus.HardRectangle cyclingOverlapOffsetSize -float 16
```

By default, only one cascade layer is shown (the original window plus one offset). To allow more layers:

```bash
defaults write com.teamtumnus.HardRectangle cyclingOverlapMaxCascade -int 3
```
...
```bash
defaults write com.teamtumnus.HardRectangle stackBadge -bool true
```
...
```bash
defaults write com.teamtumnus.HardRectangle moveFixedSizeToEdge -int 1  # align edges and corners (default)
defaults write com.teamtumnus.HardRectangle moveFixedSizeToEdge -int 2  # align corners only, center halves/sides
defaults write com.teamtumnus.HardRectangle moveFixedSizeToEdge -int 3  # center within the snap area
```
